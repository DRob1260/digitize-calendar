...
for i in range(len(rows) - 1):
    min_length = min(len(rows[i]), len(rows[i+1])) - 1  # use the shorter row
    for j in range(min_length):
        x1, y1 = rows[i][j]
        x2, y2 = rows[i+1][j+1]

        # Ensure valid coordinates and slice size
        if x2 > x1 and y2 > y1:
            # Crop the cell
            ...
            if cell.size > 0:
                # Save image
                filename = f"{days_folder}/day_{day_num:02d}.jpg"
                ...
                print(f"Skipping empty slice for day {day_num}")
        else:
            print(f"Invalid coordinates for day {day_num}")
